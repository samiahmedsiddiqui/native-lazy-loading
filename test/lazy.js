...
describe('Test `loading="lazy"` attribute', () => {
  it('Add loading="lazy" attribute on all images and iFrames', () => {
    const expectedOutput = fs.readFileSync(
      path.resolve(__dirname, 'expected-output/lazy-all.html'),
      'utf8',
      (err, data) => {
        if (err) {
          console.error(err)
    ...
    assert.strictEqual(nativeLazyLoading(testHTML), expectedOutput);
  });

  it('Add loading="lazy" attribute on all images and iFrames except that contains no-lazy or header-images class', () => {
    const expectedOutput = fs.readFileSync(
      path.resolve(__dirname, 'expected-output/lazy-exception.html'),
      'utf8',
      (err, data) => {
        if (err) {
          console.error(err)
  ...
  it('Add loading="lazy" attribute on only images and iFrames that contains testing-lazyload class', () => {
    const expectedOutput = fs.readFileSync(
      path.resolve(__dirname, 'expected-output/particular-lazy.html'),
      'utf8',
      (err, data) => {
        if (err) {
          console.error(err)
